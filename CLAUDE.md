...
@.claude/rules/funnel-slug-rules.md
...
## Email Architecture
- PearlOS owns: transactional emails only — welcome client (convert-lead), welcome team (send-welcome), offboard farewell (offboard/finalize). Templates in lib/emails/.
- Geo-landing owns: all sequence send pipeline, crons, AI gate, graduation chains, geo_email_queue, geo_email_events. Nothing moves.
- PearlOS may write to geo_email_queue directly to trigger sequence enrollment, using the same dedup rules (partial unique index on (email, sequence, step) WHERE sent_at IS NULL AND cancelled_at IS NULL). Geo-landing cron handles all sends as normal.
- PearlOS reads geo_email_queue and geo_email_events read-only for sequences dashboard stats.
- PearlOS writes to geo_suppressed only on convert-lead with reason = client.
- This separation is permanent. Do not add send logic to PearlOS outside the three transactional emails and any queue-write triggers.
...
| CSV import | `import_v2_cold_outreach` | (none — use `buildImportTag(campaignName)`) |

### Import Tags (CSV uploads)

```typescript
import { buildImportTag } from "../../../../lib/source";
const import_tag = buildImportTag(campaign_name); // "v2-cold-outreach" → "import_v2_cold_outreach"
```
...
|---|---|---|---|---|---|
| **V2 Client** | Paying customer — a real estate agent using the CashOffer/V2 system | `v2.heypearl.io/cashoffer/[slug]` | `app/cashoffer/[slug]/leads/page.tsx` | Home **sellers** (cold outreach via v2-cold-outreach campaign) | `v2-` |
...
|---|---|---|
| V2 **client** (`cashoffer/[slug]/leads`) | `v2-cold-outreach` | Cold Seller Outreach |
...
- ✅ Duplicate booking guard — DB `UNIQUE(email, meeting_time)` constraint on `geo_scheduled_calls` + INSERT-first ordering in all booking paths. 23505 conflict = race-condition duplicate → return early before any email is queued. `skipSteps` logic removed from both Calendly webhook and `app/api/booked/route.ts` — step 2 always sends.
- ✅ Email dedup — two layers: (1) Calendly webhook INSERT-first + DB unique constraint; (2) enqueueSequence INSERT-first lock in `geo_email_events` for instant emails. `ALWAYS_RESEND` emails bypass Layer 2 (confirmations must always resend on rebooking). `geo_email_events` has `UNIQUE(email, sequence, step) WHERE event_type='sent'` index.
...
- ✅ V2 client `+ Upload` modal — unified: add individual OR upload CSV, series picker shows v2- campaigns ("Cold Seller Outreach"), file upload supported
...
- ✅ Instantly campaigns — live client/affiliate campaigns: v2-cold-outreach (ID: 34436d93-157f-47ad-be1a-2dc0d174f1da), geo-buyers, geo-sellers, aff-local, aff-geo, aff-v2. Super admin campaigns: GEO Real Estate Agent Outreach (legacy name), god-v2 (rename → super-admin-v2), god-local (rename → super-admin-local), aff-recruit. Old v2-expired-listings, v2-pre-foreclosures, v2-probate deleted 2026-04-06.
...
- ✅ Unified accounts system (Phase 1+2) — `accounts` + `account_offers` tables, `lib/account.ts`, `pearlos_auth` cookie (`{accountId}:{sessionToken}`, httpOnly, 30-day). Portal at `geo.heypearl.io/account/*`: login, dashboard (offer cards + profile settings). Admin at `/admin/accounts`. API routes: `/api/account/login|logout|me|profile|upload-headshot`, `/api/admin/accounts` + `[id]` + `[id]/offers`. Phase 3 (migrate existing affiliates, dual-cookie acceptance) not yet built.
