...
const OFFER_PREFIXES: Record<string, string> = {
  ...
  local:          "local",
  affiliate:      "aff",
  "super-admin":  "super-admin", // super admin's own outreach campaigns + aff-recruit
};
...
export async function GET(req: NextRequest) {
  ...
  // Super admin campaigns: new naming is super-admin-[offer], legacy naming is god-[offer]
  // Known campaigns that don't follow a prefix pattern
  const SUPER_ADMIN_CAMPAIGN_NAMES = new Set([
    "geo — real estate agent outreach",
    "aff-recruit",
  ]);

  if (offer === "super-admin") {
    campaigns = campaigns.filter(c =>
      c.name.toLowerCase().startsWith("super-admin-") ||
      c.name.toLowerCase().startsWith("god-") || // legacy names — rename in Instantly to super-admin-*
      SUPER_ADMIN_CAMPAIGN_NAMES.has(c.name.toLowerCase())
    );
  } else if (offer && OFFER_PREFIXES[offer]) {
