...
**The law:** Admin = god. Every `/leads` portal for every offer (GEO, V2, Local, any future offer) = replica of admin minus admin-only features. When a feature ships in admin, it ships everywhere. No exceptions, no per-portal variations.
...
### Config System
- [ ] Create `app/templates/[offer]/configs/demo.ts` — god demo config
...
- [ ] `app/[offer]/[slug]/leads/page.tsx` — god template for all clients (one file, all clients inherit)
...
- [ ] `app/admin/[offer]-leads/page.tsx` — god view of all leads for this offer
...
- [ ] Create Instantly campaigns with `[offer]-` prefix for clients, `aff-[offer]` for affiliates
...
### Config
- [ ] Create `app/templates/cashoffer/configs/[slug].ts` — spread from `demo.ts`, override identity fields
- [ ] Add to `app/templates/cashoffer/configs/index.ts` registry

### Client Verification
- [ ] Client visits invite link → sets password → lands on leads portal
- [ ] Client can upload CSV and see the correct campaign labels (Expired Sellers, Pre-Foreclosure Sellers, Probate Sellers)
...
- [ ] Never hardcode marketing copy, services, FAQs, steps, testimonials — those come from the god
