...
export async function GET(req: NextRequest) {
  ...
  for (const call of calls) {
    ...
    if (match && await isTrueShow(match)) {
      ...
      // Cancel remaining post_booking, enqueue post_call
    ...
    // Store custom email so the send cron can use it for post_call step 1
    if (customEmailHtml) {
      await supabase.from("geo_email_queue")
        .update({ metadata: { custom_html: customEmailHtml } })
        .eq("email", call.email)
