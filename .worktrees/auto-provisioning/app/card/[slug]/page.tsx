...
import VapiSupportCard from "./VapiSupportCard";
import cfg from "@/app/card/config";
import { cardMetaForSlug } from "@/app/card/meta";

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  return cardMetaForSlug(slug);
}

const OFFER_CONFIG: Record<string, { label: string; section: string; url: (slug: string) => string }> = {
  geo:   { label: "GEO AI Visibility Engine",      section: "For Real Estate Agents", url: (slug) => `https://geo.heypearl.io/${slug}` },
  v2:    { label: "V2 Seller Attraction Engine",   section: "For Real Estate Agents", url: (slug) => `https://v2.heypearl.io/${slug}` },
  local: { label: "Local Business Growth Engine",  section: "For Local Businesses",  url: (slug) => `https://local.heypearl.io/${slug}` },
};
...
export default async function CardPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: affiliate } = await supabase
    .from("affiliates")
    .select("name, slug, phone, email, headshot_url, calendly_url, meta_pixel_id, offers, active")
    .eq("slug", slug)
    .maybeSingle();
  ...
  return (
    <>
      {aff.meta_pixel_id && <MetaPixel id={aff.meta_pixel_id} />}
      <div style={{
        minHeight: "100vh",
        ...
        <div style={{ width: "100%", maxWidth: 400, display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{
            ...
            <h1 style={{ color: "#ffffff", fontWeight: 800, fontSize: 24, margin: "0 0 8px", letterSpacing: "-0.5px" }}>
              {aff.name}
            </h1>
          ...
          {/* Affiliate Resources */}
          <div>
            <p style={sectionLabel}>Affiliate Resources</p>
            <div style={glassCard}>
              <a href={`https://geo.heypearl.io/${aff.slug}/leads`} target="_blank" rel="noreferrer" style={btn}>
                <span>{cfg.leadsLabel}</span>
                <span style={{ fontSize: 16, opacity: 0.4 }}>›</span>
              </a>
              <a href={cfg.communityUrl} target="_blank" rel="noreferrer" style={btn}>
                <span>{cfg.communityLabel}</span>
                <span style={{ fontSize: 16, opacity: 0.4 }}>›</span>
              </a>
            </div>
          </div>
