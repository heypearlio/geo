...
## Dedup architecture (updated Apr 2026)

The system has two dedup layers. Both must stay intact.

### Layer 1 — Calendly webhook: INSERT-before-enqueue + DB unique constraint

`geo_scheduled_calls` has a `UNIQUE(email, meeting_time)` constraint. Every booking path in `app/api/calendly-webhook/route.ts` INSERTs to `geo_scheduled_calls` BEFORE calling `enqueueSequence`. If two concurrent webhook fires race, only one INSERT wins — the other gets `code === "23505"` and returns early before any email is queued.

**Rules that must stay true:**
- Every booking path (GEO/V2, pre-interview, affiliate) MUST INSERT to `geo_scheduled_calls` BEFORE `cancelQueuedEmails` or `enqueueSequence`
- On `callInsertErr.code === "23505"` → return `{ ok: true }` immediately
- The affiliate path writes `event_type: "affiliate"`, pre-interview writes `event_type: "pre_interview"` — do not remove these
- `skipSteps` logic was removed from both calendly-webhook and `app/api/booked/route.ts` — do not re-add it. Step 2 (pre-meeting reminder) always sends regardless of prior email history.

### Layer 2 — enqueueSequence: INSERT-first lock for instant emails

For non-ALWAYS_RESEND instant emails, `enqueueSequence` now inserts the `sent` event row into `geo_email_events` BEFORE calling Resend. If `23505` → already sent → skip. This prevents double-sends from any trigger source (not just Calendly).

**ALWAYS_RESEND emails bypass Layer 2 entirely.** Re-booking confirmations, re-opt-in emails — these must always send. They rely on Layer 1 (the Calendly constraint) instead. Do not apply the INSERT-first lock to ALWAYS_RESEND.

`ALWAYS_RESEND` in `lib/email-config.ts` is NOT a defense against duplicate webhooks. Layer 1 handles that. Layer 2 handles everything else.

## Calendly assignment rules — never mix these

- **Super admin pages (all offers):** always use the master Calendly (`NEXT_PUBLIC_GEO_CALENDLY_URL`). This is Misti's by default but can be updated via Vercel env var when the master salesperson changes.
...
- These never overlap. A super admin page never uses an affiliate's Calendly. An affiliate page never uses the master Calendly. The only changes are: (1) master link is updated in Vercel, or (2) an affiliate updates their own link in their account.
