...
export default function AffiliateDetailPage() {
  ...
  async function toggleCashofferAccess() {
    if (!aff) return;
    const data = await patch({ cashofferClient: !aff.cashoffer_client });
  ...
  const checklist = [
    ...
    { label: "Social channels added", done: !!(aff.instagram_url || aff.facebook_url || aff.linkedin_url || aff.tiktok_url || aff.youtube_url) },
  ];
      ...
      <div style={{ padding: "32px 24px", maxWidth: 960, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {/* Social Channels */}
            <div style={{ background: S.card, border: `1px solid ${S.border}`, borderRadius: 12, padding: 24 }}>
              <h2 style={{ color: S.text, fontSize: 15, fontWeight: 700, marginBottom: 16, marginTop: 0 }}>Social Channels</h2>
              {[
                { label: "Instagram", value: aff.instagram_url },
                { label: "Facebook",  value: aff.facebook_url  },
                { label: "LinkedIn",  value: aff.linkedin_url  },
                { label: "TikTok",    value: aff.tiktok_url    },
                { label: "YouTube",   value: aff.youtube_url   },
              ].map(({ label, value }) => (
                <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${S.border}` }}>
                  <span style={{ color: S.muted, fontSize: 13 }}>{label}</span>
                  {value
                    ? <a href={value} target="_blank" rel="noopener noreferrer" style={{ color: S.pink, fontSize: 13, fontWeight: 500, textDecoration: "none", maxWidth: "60%", wordBreak: "break-all", textAlign: "right" }}>{value.replace(/^https?:\/\/(www\.)?/, "")}</a>
