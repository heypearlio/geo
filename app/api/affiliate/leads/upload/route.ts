...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < leads.length; i += BATCH) {
    ...
    else { console.error("Instantly affiliate upload error:", await res.text()); failed += batch.length; }
  }

  // Tag all valid leads in geo_lead_tags so they appear in affiliate + admin dashboards immediately
  if (leads.length > 0) {
    const tagRows = leads.map(l => ({ email: l.email, tag: affiliate.tag }));
    await supabase
      .from("geo_lead_tags")
      .upsert(tagRows, { onConflict: "email,tag", ignoreDuplicates: true });

    // Store name + phone in local_lead_status for enrichment
    const statusRows = leads
    ...
    if (statusRows.length > 0) {
      await supabase
        ...
        .upsert(statusRows, { onConflict: "affiliate_id,email" });
    }
  }
