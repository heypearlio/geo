...
export async function POST(req: NextRequest) {
  try {
    ...
    // Loop guard: drop anything sent from our own domains to prevent infinite forwarding
    const senderDomain = (from ?? "").split("@")[1]?.toLowerCase() ?? "";
    const BLOCKED_DOMAINS = ["heypearl.io", "geo.heypearl.io", "heypearl.co"];
    if (BLOCKED_DOMAINS.some(d => senderDomain === d || senderDomain.endsWith(`.${d}`))) {
      return NextResponse.json({ ok: true });
    }
