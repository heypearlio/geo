...
//   client_slug = the client/affiliate identifier (super admin uses "super-admin", legacy "god")
//   offer       = "v2" | "geo" | "affiliate" | "super-admin" (defaults to "v2" if omitted)
//
...
export async function POST(req: NextRequest) {
  ...
  if (offer === "v2") {
    // V2 clients + super admin v2 uploads (client_slug=super-admin or legacy client_slug=god).
    // Super admin replies appear in /admin/v2-leads which shows all cashoffer_leads regardless of slug.
    // For real clients, verify the client exists and is active first.
    const isSuperAdmin = clientSlug === "super-admin" || clientSlug === "god";
    if (!isSuperAdmin) {
    ...
    const cashoferSlug = isSuperAdmin ? "v2-cashoffer" : `v2-cashoffer-${clientSlug}`;

    const { data: existing } = await supabase
      .from("cashoffer_leads")
      .select("id")
      .eq("email", leadEmail)
      .eq("slug", cashoferSlug)
      .maybeSingle();

    if (existing) {
      console.log(`Instantly webhook: duplicate v2 lead ${leadEmail} for ${cashoferSlug} — skipped`);
      return NextResponse.json({ ok: true, duplicate: true });
    }

    const { error } = await supabase
      .from("cashoffer_leads")
      .insert({ email: leadEmail, name, slug: cashoferSlug, address: `(Instantly reply — ${eventType})` });
