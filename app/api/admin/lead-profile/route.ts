...
export async function GET(req: NextRequest) {
  ...
  const [
    ...
    supabase.from("geo_audit_history")
      .select("overall, seo, ai, website, created_at, source_tag")
      .eq("email", email)
      .order("created_at", { ascending: false })
      .limit(5),
  ...
  // Derive source — Phase 1 source_tag takes priority, fall back to old metadata.source for pre-Phase-1 leads
  const allQueueMeta = (queueRows ?? []).map(r => r.metadata as Record<string, string> | null);
  const auditSourceTag  = (auditRows ?? []).find(r => (r as typeof r & { source_tag?: string }).source_tag)?.source_tag as string | undefined;
  const localSourceTag  = (localSubmissions ?? []).find(r => r.source_tag)?.source_tag ?? undefined;
  const metaSourceTag   = allQueueMeta.find(m => m?.source_tag)?.source_tag;
  const metaSource      = allQueueMeta.find(m => m?.source)?.source;
  const firstSource     = auditSourceTag ?? localSourceTag ?? metaSourceTag ?? metaSource ?? "unknown";
