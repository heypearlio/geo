...
export async function GET(
  ...
  return NextResponse.json({
    ...
    instagram_url: affiliate.instagram_url ?? null,
    facebook_url:  affiliate.facebook_url  ?? null,
    linkedin_url:  affiliate.linkedin_url  ?? null,
    tiktok_url:    affiliate.tiktok_url    ?? null,
    youtube_url:   affiliate.youtube_url   ?? null,
  });
}
...
export async function PATCH(
  ...
  if (body.cashofferClient !== undefined) {
    ...
    if (body.cashofferClient === true) {
      ...
      if (aff) {
        ...
        if (!existing) {
          ...
          if (insertError) {
            return NextResponse.json(
              { error: "Failed to provision cashoffer access: shell record not created." },
              { status: 500 }
            );
          }
        }
    ...
    // Never delete the v2_clients record — lead data is sacred.
  }
