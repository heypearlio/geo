...
export async function GET(req: NextRequest) {
  ...
  for (const row of pending ?? []) {
    try {
      ...
      if (data.status === "FAILED") {
        ...
        // Insert each step individually so a conflict on one step (dedup index)
        // doesn't silently drop the other steps — same pattern as enqueueSequence.
        ...
        for (let i = 0; i < delays.length; i++) {
          const { error } = await supabase.from("geo_email_queue").insert({
            ...
            step: i + 1,
            send_at: new Date(now.getTime() + delays[i] * 60 * 60 * 1000).toISOString(),
          });
  ...
  Sentry.captureCheckIn({ monitorSlug: "audit-emails-cron", status: "ok", checkInId });
