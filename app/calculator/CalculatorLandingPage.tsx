...
import SellerQuiz from "./SellerQuiz";
...
export default function CalculatorLandingPage({ socialUrls, clientSlug }: { socialUrls?: SocialUrls; clientSlug?: string }) {
    ...
    function handleMessage(event: MessageEvent) {
      ...
      if (event.data?.type === "sellerhq:lead-captured" && !capturedRef.current) {
        ...
        fetch("/api/calculator-optin", {
          ...
          body: JSON.stringify({ email, name, phone, clientSlug }),
        }).catch(() => {});
      }

      if (event.data?.type === "sellerhq:book-call") {
        router.push(clientSlug ? `/cashoffer/${clientSlug}/schedule` : "/schedule");
      }
    }
  ...
  return (
    <main className="min-h-screen font-sans">
      ...
      {/* 4. CALCULATOR — DARK */}
      <section id="calculator" className="bg-[#080F1E] px-6 py-12 md:py-20">
        <div className="max-w-2xl mx-auto">

          {/* Section label */}
          <div className="text-center mb-8">
            <p className="text-[#16A34A] text-xs font-bold uppercase tracking-widest mb-3">Your Numbers</p>
            <h2 className="text-white text-3xl md:text-4xl font-extrabold leading-tight mb-3">
              {showCalculator ? "Run Your Scenarios" : "Let\u2019s Start With You"}
            </h2>
            <p className="text-[#9BACC0] text-sm">
              {showCalculator
                ? "Every number updates live as you adjust."
                : "Three quick questions before the numbers. Takes 30 seconds."}
            </p>
          </div>
          ...
          {!showCalculator ? (
            <div className="max-w-lg mx-auto">
              <SellerQuiz onComplete={() => setShowCalculator(true)} />
            </div>
          ) : (
            <div
              style={{
            ...
            >
              ...
              {SELLERHQ_URL ? (
                ...
                <div className="flex items-center justify-center h-64 bg-white/5 rounded-3xl border border-white/10">
